// Cyberpunk Glassmorphic Auth, Registration & PIN Verification Modal
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handlePinSubmit = async (e: React.FormEvent) => {
    ...
    const success = await authenticateWithPin(selectedProfileForPin.id, enteredPin);
    if (success) {
      setCurrentUserState(selectedProfileForPin);
      onUserChanged(selectedProfileForPin);
      setSelectedProfileForPin(null);
      onClose();
    } else {
      setPinError(true);
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-2xl pointer-events-auto"
      ...
      >
        <motion.div
          ...
          className="bg-zinc-950 border border-white/20 p-6 md:p-8 max-w-xl w-full shadow-[0_0_90px_rgba(34,211,238,0.2)] max-h-[92vh] overflow-y-auto rounded-3xl space-y-6 custom-scrollbar"
        >
          {/* Header */}
          <div className="flex items-center justify-between border-b border-white/10 pb-4">
            <div className="flex items-center gap-3.5">
              ...
              <div>
                ...
                <p className="text-xs text-white/50">Secure profile authentication & session partition</p>
              </div>
            </div>
            {!isMandatory && (
              <button
          ...
          {!selectedProfileForPin && (
            <div className="flex bg-black/50 p-1 rounded-2xl border border-white/10">
              ...
              >
                <User className="w-3.5 h-3.5" /> Sign In ({profiles.length})
              </button>
              <button
          ...
          {/* PIN UNLOCK VIEW */}
          {selectedProfileForPin ? (
            <form onSubmit={handlePinSubmit} className="space-y-5 p-2">
              <div className="text-center space-y-2">
                <img
                  src={selectedProfileForPin.avatarUrl || `https://api.dicebear.com/7.x/bottts/svg?seed=${encodeURIComponent(selectedProfileForPin.name)}`}
                  alt={selectedProfileForPin.name}
                  className="w-16 h-16 rounded-2xl mx-auto border-2 border-cyan-500/50 p-1 bg-black shadow-[0_0_25px_rgba(34,211,238,0.25)]"
                />
                <h3 className="text-base font-bold text-white">Unlock {selectedProfileForPin.name}'s Profile</h3>
                <p className="text-xs text-white/50">Enter the 4-digit security PIN to access interview scores</p>
                {selectedProfileForPin.id === 'user_jamadagni' && (
                  <span className="inline-block px-2 py-0.5 rounded bg-cyan-500/10 border border-cyan-500/20 text-[10px] font-mono text-cyan-300">
                    Default Demo PIN: 7777
                  </span>
                )}
              </div>
              ...
              <div>
                <div className="relative max-w-xs mx-auto">
                  <KeyRound className="w-5 h-5 absolute left-4 top-3.5 text-cyan-400" />
                  <input
                    type="password"
                    maxLength={8}
                    autoFocus
                    ...
                    className="w-full bg-white/5 border border-white/20 rounded-2xl py-3 pl-12 pr-4 text-center text-sm font-mono tracking-widest text-white placeholder:text-white/30 outline-none focus:border-cyan-400 transition-all"
                  />
                </div>
                {pinError && (
                  <p className="text-center text-rose-400 text-xs mt-2 font-mono">
              ...
              <div className="flex gap-3 max-w-xs mx-auto">
                <button
                  ...
                  className="flex-1 py-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-white/60 text-xs font-bold transition-all"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={!enteredPin}
                  className="flex-1 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white text-xs font-bold transition-all shadow-lg shadow-cyan-500/20 disabled:opacity-40"
                >
                  Unlock & Enter
                </button>
              </div>
                ...
                <div className="space-y-4">
                        ...
                        >
                          <div className="flex items-center gap-3.5">
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                {p.pin && (
                                  <Lock className="w-3.5 h-3.5 text-amber-400" title="PIN Protected" />
                                )}
                                {isActive && (
                          ...
                          <div className="flex items-center gap-2">
                            {isActive ? (
                              ...
                              <div className="px-3 py-1.5 rounded-xl bg-white/5 border border-white/10 text-[11px] font-bold text-cyan-300 group-hover:bg-cyan-500/20 group-hover:border-cyan-500/40 transition-all flex items-center gap-1">
                                {p.pin ? <KeyRound className="w-3 h-3 text-amber-400" /> : null}
                                <span>Sign In</span>
                              </div>
                            )}
                          </div>
                  ...
                  <div className="pt-3 border-t border-white/10 flex flex-col sm:flex-row gap-2.5">
                    ...
                    >
                      ...
                      <span>Guest Mode (No History)</span>
                    </button>
                    {currentUser && (
                      <button
                        onClick={handleLogout}
                        className="py-3 px-4 rounded-2xl bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 text-rose-300 text-xs font-bold transition-all flex items-center justify-center gap-2"
                      >
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          type="password"
                          maxLength={8}
                          placeholder="4-digit PIN (e.g. 7777)"
                          value={pin}
                          onChange={(e) => setPin(e.target.value)}
                          className="w-full bg-white/5 border border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-xs font-mono text-white placeholder:text-white/30 outline-none focus:border-purple-500/60 transition-all"
                        />
                      </div>
                    </div>
                  </div>
                ...
                <form onSubmit={handleUpdate} className="space-y-4">
                  <div className="flex items-center gap-4 p-4 rounded-2xl bg-black/40 border border-white/10">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] font-mono text-cyan-400">Role: {currentUser.role}</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={pin}
                        onChange={(e) => setPin(e.target.value)}
                        className="w-full bg-white/5 border border-white/10 rounded-xl py-2.5 px-4 text-xs font-mono text-white placeholder:text-white/30 outline-none focus:border-pink-500/60 transition-all"
                      />
                    </div>
                  </div>
